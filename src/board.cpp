...
void Board::print(const tetromino& t) const {
    // Primeira: criar uma cópia do grid como um array temporário
    int display[BOARD_HEIGHT][BOARD_WIDTH];
    for(int i=0; i<BOARD_HEIGHT; ++i)
        for(int j=0; j<BOARD_WIDTH; ++j)
            display[i][j] = grid[i][j];

    // Adicionar o tetromino atual na cópia (não fixa)
    const Points* blocks = t.getBlock();
    for(int i=0;i<4;++i){
        int x = blocks[i].x + t.getGlobalPosition().x;
        int y = blocks[i].y + t.getGlobalPosition().y;
        if(x >= 0 && x < BOARD_WIDTH && y >= 0 && y < BOARD_HEIGHT)
            display[y][x] = 2; // 2 representa tetromino caindo
    }
